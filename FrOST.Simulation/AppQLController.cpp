...
static void translatePoint(float *x, float *y)
{
	*x = rhd ? *x + llx : *x - llx;
	*y = *y - llx;
}
...
{
	/** These are set here so as to optimise the drawing code. */

	/* Obtain the bounding box of the network. If the point (0,0) is passed to
	* qps_DRW_filledCircle, then the circle will be drawn at (llx, lly)....
	*/
	qpg_POS_network(&llx, &lly, &urx, &ury);

	/**
	* We need to correct x coordinates if the network is right hand drive.
	*/
	rhd = qpg_CFG_driveOnRight();
	...
	for(unsigned i=0; i < nodeConfig.size(); i++)
	{
		...
		JunctionCore* junctionX = new JunctionCore(); //create new junction 
		...
		base = "u"+junctionX->id;
		temp = base + "NS";tdet = const_cast<char*>(temp.c_str());
		junctionX->upstreamDetectors[0] = qpg_NET_detector(tdet);
		temp = base + "EW";tdet = const_cast<char*>(temp.c_str());
		junctionX->upstreamDetectors[1] = qpg_NET_detector(tdet);
		temp = base + "SN";tdet = const_cast<char*>(temp.c_str());
		junctionX->upstreamDetectors[2] = qpg_NET_detector(tdet);
		temp = base + "WE";tdet = const_cast<char*>(temp.c_str());
		junctionX->upstreamDetectors[3] = qpg_NET_detector(tdet);

		junctionX->stoplineDetectors.resize(4);
		base = "s"+junctionX->id;
		temp = base + "NS";tdet = const_cast<char*>(temp.c_str());
		junctionX->stoplineDetectors[0] = qpg_NET_detector(tdet);
		temp = base + "EW";tdet = const_cast<char*>(temp.c_str());
		junctionX->stoplineDetectors[1] = qpg_NET_detector(tdet);
		temp = base + "SN";tdet = const_cast<char*>(temp.c_str());
		junctionX->stoplineDetectors[2] = qpg_NET_detector(tdet);
		temp = base + "WE";tdet = const_cast<char*>(temp.c_str());
		...
		junctionX->agentController = APPQL::AppQLearning();
		...
		APPQL::AppQLearningPolicy::AppQLearningSTATE inState;
		...
		junctionX->agentController.getPolicy().updateApproxFeatures(inState);
